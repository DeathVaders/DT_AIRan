fprintf("    Using DT construction error level = %.0f%%\n", err_level * 100);
...
if has_rays
    ...
    % 截取路径：只保留前 max_paths 条，超出忽略，不足按实际数使用
    N_use = min(length(rays{1}), max_paths);
    fprintf("Using %d paths (max_paths = %d)\n", N_use, max_paths);
...
for mc = 1:Nmc
    if has_rays
        ...
        sigma_toa = err_level * mean(ToAs0);
        dToA = (2 * rand(size(ToAs0)) - 1) * sigma_toa;
        ToAs = max(ToAs0 + dToA, 0);
